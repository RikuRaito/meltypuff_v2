...
export default function Header({ variant = "dark" }: HeaderProps) {
  ...
  const cartBorderColor =
    variant === "dark" ? "border-white/30" : "border-black/30";
  ...
  return (
    <header className="fixed inset-x-0 top-3 z-50">
      <div
        className={`mx-auto flex w-[92%] max-w-6xl items-center justify-between gap-6 rounded-full border ${borderColor} px-6 py-3 text-sm shadow-lx backdrop-blur`}
      >
        <Link
          href="/"
        ...
        >
          <img
            ...
            className="w-10"
          />
          <span className={`text-xl tracking-wide ${textColor}`}>
            Melty Puff
          </span>
        </Link>
